...
my $config;
my $ini_filename = './config.ini';
tie %{$config}, 'Config::IniFiles', ( -file => $ini_filename );

my $map_url = $config->{config}->{map_url};

my $db_host = $config->{aprs_database}->{host};
my $db_name = $config->{aprs_database}->{database};
my $db_user = $config->{aprs_database}->{username};
my $db_pass = $config->{aprs_database}->{password};

my $loc_db_host = $config->{local_database}->{host};
my $loc_db_name = $config->{local_database}->{database};
my $loc_db_user = $config->{local_database}->{username};
my $loc_db_pass = $config->{local_database}->{password};
...
sub set_multiple_station_header
{
...
sub set_station_header
{
	my $self = shift;
	$writer->startTag('station');
	$writer->dataElement('callsign', $self->{_call});
}

sub set_station_footer
{
	$writer->endTag();
}
...
sub set_last_position
{
	...
	my $sql = qq
	{
		select packet_date, month(packet_date) as month, dayofmonth(packet_date) as day, year(packet_date) as year,
		hour(packet_date) as hour, minute(packet_date) as minute, second(packet_date) as second,
		latitude,longitude,speed,course,altitude,packet_id,symbol_code,symbol_table,status
	...
	if (!$rec)
	{
		$writer->dataElement('error', 'Not found' || '');
		$self->{_packet_id} = '';
		return;		
	}
	...
	$writer->startTag('position');
		...
		$writer->startTag('age');
			$writer->dataElement('days', abs($age->{days}));
			$writer->dataElement('hours', abs($age->{hours}));
			$writer->dataElement('minutes', abs($age->{minutes}));
			$writer->dataElement('seconds', abs($age->{seconds}));
		$writer->endTag();
		...
		$writer->endTag();
	$writer->endTag();
	# moving status comment up into this routine since it appears
	# that aprsworld no longer uses the laststatus table, and
	# status is now a field in the lastposition table
	# bam 2011-01-17
	$writer->startTag('status');
		$writer->dataElement('comment', $rec->{status} || '');
	$writer->endTag();
	$self->{_packet_date} = $rec->{packet_date};
...
sub set_last_weather
{
	...
	return if !$self->{_call};

	# looks like the packet_id's are broke and are only set to 0 now.  so switching to source='callsign'.
	# bam 7/4/2006
	#
	# my $sql = qq
	# { 
	...
	$writer->startTag('weather');
		...
		$writer->startTag('dewpoint');
			$writer->dataElement('celsius', _get_dewpoint($rec->{temperature},$rec->{humidity}) || '');
			$writer->dataElement('fahrenheit', _convert_celsius_to_fahrenheit(_get_dewpoint($rec->{temperature},$rec->{humidity})));
		$writer->endTag();
		...
		$writer->startTag('wind');
			...
			$writer->startTag('sustained');
				...
				# $writer->dataElement('mph', $rec->{wind_sustained} || '');
				# $writer->dataElement('kph', _convert_mph_to_kph($rec->{wind_sustained}));
				# $writer->dataElement('knots', _convert_mph_to_knots($rec->{wind_sustained}));
			$writer->endTag();
		$writer->endTag();
		$writer->startTag('rain');
			$writer->startTag('hour');
				$writer->dataElement('cm', $rec->{rain_hour} ? sprintf("%1.2f",$rec->{rain_hour}) : '');
				$writer->dataElement('in', _convert_cm_to_in($rec->{rain_hour}));
			$writer->endTag();
			$writer->startTag('day_calendar');
				$writer->dataElement('cm', $rec->{rain_calendar_day} ? sprintf("%1.2f",$rec->{rain_calendar_day}) : '');
				$writer->dataElement('in', _convert_cm_to_in($rec->{rain_calendar_day}));
			$writer->endTag();
			$writer->startTag('day_24hour');
				$writer->dataElement('cm', $rec->{rain_24hour_day} ?  sprintf("%1.2f",$rec->{rain_24hour_day}) : '');
...
#	changing this due to a number of complaints that the station status comment is showing
#	off duty kenwood for a lot of people, including non-existent calls
#	found that this was also due the packet_id being stored as 0 for a lot of packets.  not sure why
#	aprsworld is doing that.  switching to using combo of call and packet date
#
#	my $sql = qq
#	{
#		select 
...
#	};
	...
	my $sql = qq
	{
		select 
		comment,power,height,gain,directivity,rate
		from laststatus where source=? and packet_date=?
	};

	my $sth = $dbh->prepare($sql) || die $dbh->errstr();
	$sth->execute($self->{_call},$self->{_packet_date}) or die $DBI::errstr;
	my $rec = $sth->fetchrow_hashref;
	return if !$rec;
...
{
	my $kph = shift;
	return 0 if $kph eq '0';
	return '' if !$kph;
	return sprintf "%.0f", $kph * 0.6215;
}
...
sub _convert_celsius_to_fahrenheit
{
	my $celsius = shift;
	return '' if !$celsius;
	return sprintf "%.0f",($celsius * 1.8) + 32;
}
...
sub _get_dewpoint
{
	my ($temp,$humidity) = @_; # temperatures are in celsius
	return '' if $humidity < 1;
	my $b = (log($humidity / 100) + ((17.27 * $temp) / (237.3 + $temp))) / 17.27;
	my $dp = (237.3 * $b) / (1 - $b);
	return sprintf "%.0f", $dp;
}
